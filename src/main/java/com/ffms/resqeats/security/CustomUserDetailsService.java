 ...
 * It supports multiple identification methods including UUID, email, and phone number
...
public class CustomUserDetailsService implements UserDetailsService {
     ...
     * UUID (for JWT token validation), email, and phone number. If the user is registered
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        log.debug("Searching for user - trying UUID, then email, then phone");

        User user = tryLoadByUuid(username)
                .orElseGet(() -> {
                    log.debug("Username is not a valid UUID, trying email lookup");
     ...
     * validation where the subject might be stored as a UUID string.</p>
     *
     * @param username the string to parse as a UUID
     * @return an Optional containing the User if found, or empty if the string
     *         is not a valid UUID or no user exists with that ID
     */
    private Optional<User> tryLoadByUuid(String username) {
        try {
            UUID userId = UUID.fromString(username);
            log.debug("Username parsed as UUID: {}", userId);
            Optional<User> user = userRepository.findById(userId);
            if (user.isPresent()) {
                log.debug("User found by UUID: {}", userId);
            }
            return user;
        } catch (IllegalArgumentException e) {
            log.debug("Username '{}' is not a valid UUID format", username);
